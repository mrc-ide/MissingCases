---
title: "Calculating the proportion of not detected cases in the SARS-CoV-2 epidemic in 2020 using a prior on gamma"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Calculating the proportion of not detected cases in the SARS-CoV-2 epidemic in 2020 using a prior on gamma}
...
p
ggsave("nz_gamma.pdf", p, width = 5, height = 5)
```
```{r echo=FALSE, fig.height = 5, fig.width = 5}
p <- ggplot(data_regions) + 
  geom_point(aes(x = gamma, y = alpha, col = mu)) + 
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1), labels = scales::percent) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1), labels = scales::percent) + 
  scale_color_continuous(labels = scales::label_percent(accuracy = 1)) + 
  xlab(expression("Proportion of contacts under active surveillance (" ~ gamma ~ ")")) + 
  ylab(expression("Scaling in transmission for traced cases (" ~ alpha ~ ")")) + 
  labs(colour="Proportion of not detected infections") + 
  theme_bw() + theme(legend.position="bottom",  plot.margin = margin(20, 20, 1, 1))
p
ggsave("nz2_gamma.pdf", p, width = 5, height = 5)
```
```
