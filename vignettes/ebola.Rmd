...
```{r}
n_iter = 10000                                                             
chains_a = sample_chains(n_iter = n_iter, r1 = r1a, r2 = r2a)
chains_b = sample_chains(n_iter = n_iter, r1 = r1b, r2 = r2b)
```
...
center_a <- polylabelr::poi(region_a[,1], region_a[,2], precision = 0.001)
center_b <- polylabelr::poi(region_b[,1], region_b[,2], precision = 0.01)

data_center <- data.frame("pi" = c(center_a$x, center_b$x),
                          "phi" = c(center_a$y, center_b$y),
                          "Region" = c(1, 2))

p <- ggplot(data_regions) + 
  geom_point(aes(x = pi, y = phi, col = Region), size = 0.5, alpha = 0.5) + 
...
```{r table params}
sol_a = compute_alpha_mu1(pi = center_a$x, phi = center_a$y, r1 = r1a, r2 = r2a)
sol_b = compute_alpha_mu1(pi = center_b$x, phi = center_b$y, r1 = r1b, r2 = r2b)

gamma_a = center_a$x / (r1a + center_a$x)
gamma_b = center_b$x / (r1b + center_b$x)
...
data <- data.frame(c1 = c("$r_{2}$", "$\\pi$", "$\\alpha$", "$\\phi$", "$\\gamma$", "$\\mu_{1}$"),
                   c2 = c("Ratio of de novo cases versus cases which were contacts under active surveillance", 
                          ...
                          "Proportion of cases not detected"),
                   c3 = c(format(r2a, digit = 3), 
                          paste0(format(center_a$x*100, digit=3), "\\% (", format(quantile(chains_a$pi,0.025)*100, digit=3), ", ", format(quantile(chains_a$pi,0.975)*100, digit=3) ,")"), 
                          ...
                          paste0(format(mean(center_a$y)*100, digit=3), "\\% (", format(quantile(chains_a$phi,0.025)*100, digit=3), ", ", format(quantile(chains_a$phi,0.975)*100, digit=3) ,")"), 
                   ...
                   c4 = c(format(r2b, digit = 3), 
                          paste0(format(center_b$x*100, digit=3), "\\% (", format(quantile(chains_b$pi,0.025)*100, digit=3), ", ", format(quantile(chains_b$pi,0.975)*100, digit=3) ,")"), 
                          ...
                          paste0(format(center_b$y*100, digit=3), "\\% (", format(quantile(chains_b$phi,0.025)*100, digit=3), ", ", format(quantile(chains_b$phi,0.975)*100, digit=3) ,")"), 
