...
#' @param xmin Minimum value of uniform parameter scaling
#' @param xmax Maximum value of uniform parameter scaling
...
sample_chains <- function(n_iter, r1, r2, xmin = 0, xmax = 1, alpha = FALSE){
  # Initialise chains
  mu_chain <- NULL
  pi_chain <- NULL
  phi_chain <- NULL
  ...
  {
    # Sample  pi and phi
    pi <- stats::runif(n = 1, min = xmin, max = xmax)
    if (alpha == FALSE){
      phi <- stats::runif(n = 1, min = xmin, max = xmax)
      ...
      alpha <- stats::runif(n = 1, min = xmin, max = xmax)
      # Compute alpha and mu
